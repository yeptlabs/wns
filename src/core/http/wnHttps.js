...
module.exports = {
	...
	methods: {
		...
		init: function (config,c)
		{
			...
			this.connection=self.createServer(self.e.open);
		...
		{
			if (_config.key)
				_config.key = fs.readFileSync(self.getParent().modulePath+_config.key);
			if (_config.cert)
				_config.cert = fs.readFileSync(self.getParent().modulePath+_config.cert);

			return https.createServer(_config,self.e.open);
		},
